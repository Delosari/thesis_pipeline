...
for orden_type in ordenes:
    for arm_color in colors:
        ...
        dz.task_attributes['normalizing_flat']  = dz.reducDf.loc[index_ObjFlat, 'file_location'].values[0] + dz.reducDf.loc[index_ObjFlat, 'file_name'].values[0]
        dz.task_attributes['output']            = '{folder_output}nMasterFlat{color}_{orden_type}order.fits'.format(folder_output = dz.task_attributes['run folder'], color = dz.task_attributes['color'], orden_type=orden_type)
        ...
        dz.save_task_parameter(task = 'response', parameter = 'threshold', entry = 'flat_{order}Order_{color}_fitting_regions'.format(order=orden_type,color=arm_color))
...
dz.generate_step_pdf(indeces_print, file_address = dz.reducFolders['reduc_data'] + 'nMaster_flat', plots_type = 'flats', ext = 0, include_graph=True)
