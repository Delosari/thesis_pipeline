...
#         idces_calibframes       = (dz.reducDf.frame_tag == 'flat') & (dz.reducDf.file_location.str.contains('/raw_fits/')) & (dz.reducDf.ISIARM == color + ' arm') & (dz.target_validity_check())  
#               
...
#                     
#         #Save the data
#         reference_key1 = '{codename}_{arm_color}_{calib_file}_bytime'.format(codename = target_object, arm_color = color, calib_file = 'flat')
#         reference_key2 = '{codename}_{arm_color}_{calib_file}_calibtime'.format(codename = target_object, arm_color = color, calib_file = 'flat')
#         reference_key3 = '{codename}_{arm_color}_{calib_file}_bySeparation'.format(codename = target_object, arm_color = color, calib_file = 'flat')
#         reference_key4 = '{codename}_{arm_color}_{calib_file}_calibSeparation'.format(codename = target_object, arm_color = color, calib_file = 'flat')
...
#-----------Create the table
#Create the doc
doc = Document(dz.reducFolders['reduc_data'] + 'observation_log')
...
#Table header
table_format = 'llllllllll'  
 
#Elements we want to put in the table
idces_table     = (dz.reducDf.file_location.str.contains('/raw_fits/'))
runs            = dz.reducDf[id_originals].sort_values(['RUN']).RUN.values
file_names      = dz.reducDf[id_originals].sort_values(['RUN']).file_name.values
file_folders    = dz.reducDf[id_originals].sort_values(['RUN']).file_location.values
RA_values       = dz.reducDf[id_originals].sort_values(['RUN']).RA.values
DEC_values      = dz.reducDf[id_originals].sort_values(['RUN']).DEC.values
UT              = dz.reducDf[id_originals].sort_values(['RUN']).UT.values
objects         = dz.reducDf[id_originals].sort_values(['RUN']).frame_tag.values
color           = dz.reducDf[id_originals].sort_values(['RUN']).ISIARM.values
valid_values    = dz.reducDf[id_originals].sort_values(['RUN']).valid_file.values
  
#Fill the table
with doc.create(Tabular(table_format)) as table:
        
    #Adding the header
    headers     = ['RUN', 'Object', 'Frame', 'Arm', 'ExpTime', 'Airmass', 'Slit', 'UT', 'RA', 'DEC']
...
#             color_row = 'black'
        ...
        fits_address = dz.Catalogue_folder + '/raw_fits/' + files_names[i]

        header1 = fits.getheader(fits_address, ext=0)
#         for coso in header1:
#             print coso, header1[coso]
        
       
#         target_Exptime      = getval(fits_address, 'EXPTIME', 0)
#         target_airmass      = getval(fits_address, 'AIRMASS', 0)
#         target_slitwidth    = getval(fits_address, 'ISISLITW', 1)
        
        target_Exptime = header1['EXPTIME']
        target_airmass = header1['AIRMASS']
        
        if 'ISISLITW' in header1:
            target_slitwidth = header1['ISISLITW']
        else:
            target_slitwidth = 'none'
        
        
        #Add the row
        row_values[0] = runs[i]
        row_values[1] = entry_object_type
        row_values[2] = r'\textcolor{{{color}}}{{{filename}}}'.format(color = 'black', filename=files_names[i])
        row_values[3] = '{arm_color}'.format(arm_color=color[i])
        row_values[4] = target_Exptime
        row_values[5] = target_airmass
        row_values[6] = target_slitwidth
        row_values[7] = UT[i]
        row_values[8] = RA_values[i]
        row_values[9] = DEC_values[i]
