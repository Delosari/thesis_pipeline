...
#Define operation
Catalogue_Dic                               = DataToTreat('WHT_CandiatesObjects_2016A_Marta')
...
for i in range(len(Candiates_frame.index)):
    ...
    try:
        ...
        print CodeName, mjd, plate, fiberID
        ...
        if obj_table != None:
            ...
            pv.SaveParameter_ObjLog(CodeName, FileFolder, 'SDSS_MJD', mjd)
            pv.SaveParameter_ObjLog(CodeName, FileFolder, 'SDSS_Plate', plate)
            pv.SaveParameter_ObjLog(CodeName, FileFolder, 'SDSS_Fiber', fiberID)
