...
def Load_lmfit_parameters(N_comps, Initial_guesses_dic, wide_component = False, mu_precission = 1):
    ...
    params['Flux1'].expr = 'Flux3 / 3'

    #One wide component
    if wide_component:
        print 'Este guess', Initial_guesses_dic['mu'][N_comps] - mu_precission, Initial_guesses_dic['mu'][N_comps] + mu_precission
        index = str(N_comps + 1)
        params.add('A'      + index, value = Initial_guesses_dic['A'][N_comps], min = 0)
        params.add('mu'     + index, value = Initial_guesses_dic['mu'][N_comps], min = Initial_guesses_dic['mu'][N_comps] - mu_precission, max = Initial_guesses_dic['mu'][N_comps] + mu_precission)
#         params.add('mu'     + index, expr = 'mu2')
    ...
    if wide_component:
        ...
        params.add('mu'     + index, value = Initial_guesses_dic['mu'][N_comps] - 6, vary = False)
        params.add('sigma'  + index, value = Initial_guesses_dic['sigma'][N_comps], min = Initial_guesses_dic['min_sigma'][N_comps], max = 8.0)
        params.add('fwhm'   + index, expr = '2.354820045 * {sigma}'.format(sigma = 'sigma'  + index))
        params.add('FWHM'   + index, expr = '({fwhm}/{mu}) * 2.99792458e5'.format(fwhm = 'fwhm' + index, mu = 'mu' + index))
        params.add('Flux'   + index, expr = '({A}*{fwhm})/(2.35*0.3989)'.format(A = 'A'  + index, fwhm = 'fwhm' + index))
...
for i in range(Ncomps + 2):    
    index   = str(i + 1)
    A       = out.params['A' + index].value 
    mu      = out.params['mu' + index].value 
    sigma   = out.params['sigma' + index].value 
    plt.plot(x_resample, Gaussian_Curve(A, mu, sigma, x_resample, lineal_resample), '--', color = 'blue', label = 'Components')
