...
def CompositeModel_dict(A, mu, sigma, x, zerolev):
...
#    sigma5 = params['sigma5'].value 
    ...
    return A1 * np.exp(-(x-mu1)*(x-mu1)/(2*sigma1*sigma1)) + A2 * np.exp(-(x-mu2)*(x-mu2)/(2*sigma2*sigma2)) + A3 * np.exp(-(x-mu3)*(x-mu3)/(2*sigma3*sigma3)) + A4 * np.exp(-(x-mu4)*(x-mu4)/(2*sigma4*sigma4)) + zerolev
#     return A1 * np.exp(-(x-mu1)*(x-mu1)/(2*sigma1*sigma1)) + A2 * np.exp(-(x-mu2)*(x-mu2)/(2*sigma2*sigma2)) + A3 * np.exp(-(x-mu3)*(x-mu3)/(2*sigma3*sigma3)) + zerolev
...
params.add('sigma3',    value = 4.0, min = 0, vary=True)
...
# params.add('sigma1',    value = 2.23, min = 2.0,  max = 5.0, vary=False)
...
plt.plot(x_resample, CompositeModel(out.params, x_resample), 'r--', label = 'Fitted line')
...
for i in range(3 + 1):
    ...
    plt.plot(x_resample, CompositeModel_dict(A, mu, sigma, x_resample, lineal_resample), '-', color = 'blue', label = 'Component ' + index)
