...
def Residuals_Kmpfit(p, data):
    ...
    #-----------------------------------------------------------------------
    x, y, err = data
    return (y-Gaussian_Kmpfit(p,x)) / err

def ParDerivatives_GaussianKmpfit(p, data, dflags):
    #-----------------------------------------------------------------------
...
p0 = [9, 4.5, 0.8, 0]
y = Gaussian_Kmpfit(truepars, x) + 0.3*numpy.random.randn(len(x))
err = 0.3*numpy.random.randn(N)

# The fit
fitobj = kmpfit.Fitter(residuals=Residuals_Kmpfit, deriv=ParDerivatives_GaussianKmpfit, data=(x, y, err))
try:
...
frame.plot(x, Gaussian_Kmpfit(truepars,x), 'r', label="True data")
frame.plot(x, Gaussian_Kmpfit(fitobj.params,x), 'b', lw=2, label="Fit with kmpfit")
frame.set_xlabel("X")
