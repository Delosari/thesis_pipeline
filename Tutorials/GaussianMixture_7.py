...
clf = mixture.GMM(n_components=len(Gaussian_Components), covariance_type='full')
clf.fit(y)

p_1_clg_Norm = zeros(Number_Gaussians * 3)
for i in range(Number_Gaussians):
...
for i in range(Number_Gaussians):
    ...
    print Dictionary_Conditions[i*3:(i+1)*3]
    print {}, {'limits':(mu_i-0.5,mu_i+0.5)}, {}
    Dictionary_Conditions[i*3:(i+1)*3] = {}, {'limits':(mu_i-0.01,mu_i+0.01)}, {}
print 'current dictionary', Dictionary_Conditions
...
# Axis.plot(Wave_Gaussian_clg, Flux_Gaussian_clg, color='violet',  label="clg method mixture")
Flux_Kmpfit = my_model(p1_kmpfit, Wave_Gaussian, Number_Gaussians)

print 'Wave gaussian', len(Wave_Gaussian)
print 'Flux kmpfit',   len(Flux_Kmpfit)

Axis.plot(Wave_Gaussian, Flux_Kmpfit + zerolev_Gaussian, color='violet',  label="kmpfit")

for i in range(Number_Gaussians):
    A_i, mu_i, sigma_i = p_1[i*3:(i+1)*3]
    Wave_Gaussian_i, Flux_Gaussian_i, zerolev_Gaussian_i = ResampleGaussian([A_i, mu_i, sigma_i], Wave[Ind_left:Ind_right], Continuum[Ind_left:Ind_right])
