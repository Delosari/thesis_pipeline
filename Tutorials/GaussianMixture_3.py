from numpy                  import searchsorted, array, asarray, argsort, diff, concatenate, int8, where, zeros, argmin, max, median, round, exp, linspace, ones, log10, power, min
...
def Estimage_Sigmas(Wave, Flux, A_Values, mu_Values, Emission_Edges, Number_Gaussians):

    sigma_Values = zeros(Number_Gaussians)
                    
    #Calculating the sigmas:
    Interest_Points = [Emission_Edges[0], Emission_Edges[1]]
    for i in range(len(A_Values) - 1):

        Peak_Left, Peak_Right               = where(Flux == A_Values[i])[0],  where(Flux == A_Values[i + 1])[0]
        x_BetweenMaxima, y_BetweenMaxima    = Wave[Peak_Left : Peak_Right], Flux[Peak_Left: Peak_Right]
        x_m = x_BetweenMaxima[argmin(y_BetweenMaxima)]            
        Interest_Points.append(x_m)

    Interest_Points.sort()
    for i in range(len(sigma_Values)):
        sigma_Values[i] = (Interest_Points[i+1] - Interest_Points[i]) /2 / 2.335 
    
    return sigma_Values, Interest_Points
  
def NormalizeGaussian(Wave, Flux, A_Values, mu_Values, sigma_Values, Continuum, Continuum_error, Number_Gaussians, FluxTotal):
        
    Max_y   = max(A_Values)
    Mean_x  = round(median(mu_Values))
    
    maxi = max(Flux)
    a = min(log10(Flux/maxi)) *-1
...
#     sigma_zerolev_Norm = round(Continuum_error      / Max_y, 4)
    ...
    x                  = round(Wave                 - Mean_x, 3)
    y                  = round(log10(Flux/maxi)/a + 1, 5)
    A_Norm             = round(log10(A_Values/maxi)/a + 1, 5)
    mu_Norm            = round(mu_Values            - Mean_x, 5)
    sigma_Norm         = round(sigma_Values                 , 5)
    zerolev_Norm       = round(log10(Continuum/maxi)/a + 1, 5)
    sigma_zerolev_Norm = round(Continuum_error      / Max_y, 5)

    print 'in log A...',    round(log10(A_Values/maxi)/a + 1, 5)
    print 'in log y...',    round(log10(Flux/maxi)/a + 1, 5)
    print 'in log cont',    round(log10(Continuum/maxi)/a + 1, 5)
    print ' y asi?',        round(Continuum_error/Max_y, 5)
...
def Rescale_GaussianParameters(p_1_Norm, A_Values, mu_Values, Number_Gaussians):
    ...
    for i in range(Number_Gaussians):
        ...
        p_1[i*3:(i+1)*3]            = maxi * power(10,(a * (A-1))), mu + Mean_x, sigma
...
x, y, p_0, continuum_norm, continuum_error_norm     = NormalizeGaussian(Wave_Region, Flux_Region, A_Values, mu_Values, sigma_Values, Continuum_Region, Continuum_error, Number_Gaussians, FluxTotal=Flux)
...
Fig.set_facecolor('w')
maxi = max(Flux_Region)
a = min(log10(Flux_Region/maxi)) *-1

# print ' the maxi is', maxi
print 'a is', a
print 'error is', Continuum_error / maxi
print 'and this flux is', log10(Flux_Region/maxi)/a + 1
# Axis.plot(Wave, log10(Flux/maxi)/a + 1, color='Blue', label="Spectrum")
# Axis.plot(Wave_Region, log10(Continuum_Region/maxi)/a + 1, color='Blue', label="Spectrum")
# Axis.plot(Wave_Region, log10(Flux_Region/maxi)/a + 1, color='orange', label="Spectrum")




Axis.plot(Wave, Flux, color='Blue', label="Spectrum")
Axis.plot(Wave_Gaussian, Flux_Gaussian, color='Red',  label="Fitted Gaussian")
...
for i in range(Number_Gaussians):
    ...
    Axis.plot(Wave_Gaussian_i, Flux_Gaussian_i, color='Red', linestyle=':')
