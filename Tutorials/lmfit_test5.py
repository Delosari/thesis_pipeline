...
def Load_lmfit_parameters(N_comps, Initial_guesses_dic, wide_component = False, mu_precission = 1):
    ...
    for i in range(1, N_comps):
        index = str(i + 1)
        params['sigma'      + index].expr='sigma1'
# 
    #Set flux in N2_6548A as 1/3 N2_6584A
    params.add('Flux1', expr = 'Flux3 / {ratio}'.format(ratio = N2_6584A / N2_6548A))
...
#     scale_params = OrderedDict()
    ...
    for i in range(N_comps):
        index = str(i + 1)
        scale_params.add('A'    + index, value = fit_output['A'+ index].value * y_scale)
        scale_params.add('mu'   + index, value = fit_output['mu'+ index].value + x_scale)
        scale_params.add('sigma'+ index, value = fit_output['sigma'+ index])
        scale_params.add('fwhm' + index, value = fit_output['fwhm'+ index])
        scale_params.add('FWHM' + index, value = scale_params['fwhm'+ index].value / scale_params['mu'+ index].value * 2.99792458e5)
        scale_params.add('Flux' + index, value = (scale_params['A'+index].value * scale_params['fwhm'+index].value) / (2.35 * 0.3989))
    
        print 'A'    + index,  scale_params['A'    + index].value
        print 'mu'    + index,  scale_params['mu'    + index].value
        print 'sigma'    + index,  scale_params['sigma'    + index].value
...
#Make the fitting
Fit_output = minimize(CompResid_zerolev, params, args=(line_wave, line_flux, lineal_zerolev, Ncomps + 1, err_continuum))
report_fit(Fit_output.params)


#Rescale the parameters
scale_params = rescale_parameters(Fit_output.params, Max_wavelength, Max_Flux, Ncomps, wide_component = True)
...
plt.plot(x_resample, CompositeModel_zerolev(scale_params, x_resample, lineal_resample, Ncomps + 1), 'r-', label = 'Fitted line')
...
for i in range(Ncomps + 1):    
    index   = str(i + 1)
    A       = scale_params['A' + index].value 
    mu      = scale_params['mu' + index].value 
    sigma   = scale_params['sigma' + index].value 
    plt.plot(x_resample, Gaussian_Curve(A, mu, sigma, x_resample, lineal_resample), '--', color = 'blue', label = 'Gaussian components' if i == 0 else "")
