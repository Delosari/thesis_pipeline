from numpy                  import searchsorted, array, asarray, argsort, diff, concatenate, int8, where, zeros, max, median, round, exp, linspace, ones, mean, sqrt, exp
...
def NormalizeGaussian(Wave, Flux, A_Values, mu_Values, sigma_Values, Continuum, Continuum_error, Number_Gaussians):
    ...
    h3_Norm            = zeros(Number_Gaussians) 
    h4_Norm            = zeros(Number_Gaussians) 

    p_0 = zeros(Number_Gaussians * 3)
    for i in range(Number_Gaussians):
        p_0[i*5:(i+1)*5] = A_Norm[i], mu_Norm[i], sigma_Norm[i], h3_Norm, h4_Norm

    Kmpfit_Dictionary   = []    
    A_max               = max(A_Norm)
    
    for i in range(Number_Gaussians):
        if A_Norm[i] < 0.40 * A_max:
            print i, 'pequeno'
            Kmpfit_Dictionary.append({})
            Kmpfit_Dictionary.append({'limits':(mu_Norm[i]-0.25,mu_Norm[i]+0.25)})
            Kmpfit_Dictionary.append({'limits':(0,1.2)})
            Kmpfit_Dictionary.append({})
            Kmpfit_Dictionary.append({})
        else:
            print i, 'ande'
            Kmpfit_Dictionary.append({})
            Kmpfit_Dictionary.append({})
            Kmpfit_Dictionary.append({'limits':(0,4)})
            Kmpfit_Dictionary.append({})
            Kmpfit_Dictionary.append({})
            
    return x, y, p_0, zerolev_Norm, sigma_zerolev_Norm, Kmpfit_Dictionary

# def Residuals_GaussianMixture(p, data):
#     x, y, zerolev, sigma_zerolev = data[0], data[1], data[2], data[3]
#     return (y - (MixtureGaussian_Cont_v2((x, zerolev), p))) / sigma_zerolev

# def MixtureGaussian_Cont_v2(Ind_Variables, p):
#     y = 0.0
...
def gausshermiteh3h4(x, A, x0, s, h3, h4):
    ...
    F = (x-x0)/s
    E = A*exp(-0.5*F*F)*( 1.0 + h3*F*(c3*F*F+c1) + h4*(c0+F*F*(c2+c4*F*F)) )
    return E
...
x, y, p_0, continuum_norm, continuum_error_norm, Kmpfit_Dictionary = NormalizeGaussian(Wave_Region, Flux_Region, A_Values, mu_Values, sigma_Values, Continuum_Region, Continuum_error, Number_Gaussians)

#Use kmpfit to perform the fitting on Mixture Gaussian method
fitobj              = kmpfit.Fitter(residuals=residualsGH, data=([x, y, continuum_norm, continuum_error_norm]))

fitobj.parinfo      = Kmpfit_Dictionary
fitobj.fit(params0  = p_0)
p_1_Norm            = fitobj.params 
