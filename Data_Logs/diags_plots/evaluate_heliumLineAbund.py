...
def plot_metallic_relation(catalogue_df, lines_ratio, theo_value, element, color, label):
    ...
    for i in range(len(catalogue_df.index)):
        ...
        n_e = catalogue_df.iloc[i].neSII if catalogue_df.iloc[i].neSII != nan else 100.0   
...
#     n_e = catalogue_df.iloc[i].ne_SII if catalogue_df.iloc[i].ne_SII != None else 100.0  
    ...
    for i in range(len(catalogue_df.index)):
        ...
        objName             = catalogue_df.iloc[i].name
        ouput_folder        = '{}{}/'.format(catalogue_dict['Obj_Folder'], objName) 
        lineslog_address    = '{objfolder}{codeName}_WHT_linesLog_emission_2nd.txt'.format(objfolder = ouput_folder, codeName=objName)
        ...
        HeI_idx = (lineslog_frame.Ion.str.contains('HeI_')) & (~lineslog_frame.index.isin(['He1_8446A', 'He1_9464A', 'He1_5016A', 'He1_7281A', 'He1_4713A', 'He1_4922A', 'He1_3889A']))
...
#Back up copy from different elements
