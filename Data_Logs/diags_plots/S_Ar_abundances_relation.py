from libraries.cloudy_library.cloudy_methods    import Cloudy_Tools
from collections                                import OrderedDict
from lmfit.models                               import LinearModel
from numpy                                      import log10 as nplog10, linspace, array, hstack, max as np_max, isinf
...
Lineal_guess        = lineal_mod.guess(y_linealFitting, x=x_linealFitting)
Lineal_output       = lineal_mod.fit(y_linealFitting, Lineal_guess, x=x_linealFitting)
Lineal_params       = Lineal_output.params
x_lineal            = linspace(0, np_max(x_linealFitting), 100)
y_lineal            = Lineal_params['lineal_slope'].value * x_lineal + Lineal_params['lineal_intercept'].value
...
formula = r"$log\left(Ar^{{+2}}/Ar^{{+3}}\right) = {m} \cdot log\left(S^{{+2}}/S^{{+3}}\right) + {n}$".format(m=round(Lineal_guess['lineal_slope'].value,3), n=round(Lineal_guess['lineal_intercept'].value, 3))
