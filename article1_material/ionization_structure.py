...
size_dict = {'figure.figsize':(10,10), 'axes.labelsize':24, 'legend.fontsize':14, 'font.family':'Times New Roman', 'mathtext.default':'regular', 'xtick.labelsize':22, 'ytick.labelsize':22}
...
for i in range(len(file_name_list_S)):
    ...
    for j in range(len(ions_list_S)):
        ...
        dz.data_plot(radious/1e19, ion_frac, color=ions_colors_S[j], linestyle=line_type[i], label = r'Cluster mass {}'.format(label), linewidth = 3)
        dz.plot_text(labels_coords_S[j][i][0]/1e19, labels_coords_S[j][i][1], text = ions_labels_S[j], color = ions_colors_S[j], fontsize = 17, axis_plot=None)
    ...
    for j in range(len(ions_list_O)):
        ...
        dz.plot_text(labels_coords_O[j][i][0]/1e19, labels_coords_O[j][i][1], text = ions_labels_O[j], color = ions_colors_O[j], fontsize = 17, axis_plot=None)
