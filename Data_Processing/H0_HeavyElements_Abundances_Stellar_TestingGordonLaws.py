...
for objName in catalogue_df.index:
        ...
        for i in range(len(redding_laws)):
            ...
            reddening_law = redding_laws[i]
            cHbeta = catalogue_df.loc[objName, 'cHbeta_{}'.format(reddening_law)]
            dz.deredden_lines(lineslog_frame, reddening_curve=redding_laws[i], cHbeta=cHbeta, R_v=R_v)
            ...
            dz.store_abundances_excel(objName, catalogue_df, extension = extensioin_table[i])
