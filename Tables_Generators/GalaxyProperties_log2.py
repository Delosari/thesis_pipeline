import pyneb                                as pn
import numpy                                as np
from numpy                                  import power, where, ndarray, asscalar, std, mean, array
...
def colorEvaluator(Inf_Name, Inf_Variable, CodeName, FileFolder):
  
    if Inf_Name == 'y_plus_inf':
        VariableCompare = 'HeII_HII'
        
    elif Inf_Name == 'Te_inf':
        VariableCompare = 'TSIII'
        
    elif Inf_Name == 'ne_inf':
        VariableCompare = 'nSII'
        
    elif Inf_Name == 'cHbeta_inf':
        VariableCompare = 'cHBeta'
    else:
        VariableCompare = None
        
    if VariableCompare != None:
        Obs_Variable = pv.GetParameter_ObjLog(CodeName, FileFolder, VariableCompare, 'float')
        Color =  colorChooser(Obs_Variable, Inf_Variable)
    else:
        Color = 'Black'
...
#     header_dict['S3_ratio']     = r'$\frac{\left[SIII\right]9531\AA}{\left[SIII\right]9069\AA}$'
    ...
    header_dict['cHBeta']       = r'$c\left(H\beta\right)$'
    ...
    header_dict['Y_Mass_O']     = r'$Y_{\left(\frac{O}{H}\right)}$'
    header_dict['Y_Mass_S']     = r'$Y_{\left(\frac{S}{H}\right)}$'
  
    header_dict['Y_Inference_O']= r'$Y_{\left(\frac{O}{H}\right),\,inf}$'
    header_dict['Y_Inference_S']= r'$Y_{\left(\frac{S}{H}\right),\,inf}$'
    
    #Physical Parameters
    header_dict['y_plus_inf']    = r'$\left(\frac{HeI}{HI}\right)_{inf}$'
    header_dict['Te_inf']        = r'$T_{e,\,inf}$'
    header_dict['ne_inf']        = r'$n_{e,\,inf}$'
    header_dict['cHbeta_inf']    = r'$c\left(H\beta\right)_{inf}$'
    header_dict['ftau_inf']      = r'$\tau_{\inf}$'
    header_dict['Xi_inf']        = r'$\xi_{inf}$'
...
def load_Galaxy_Ratios(FileFolder, CodeName, AbundancesExtension, Atom_Dict):
    ...
    if ('S3_9069A' in Lines_Dict) and ('S3_9531A' in Lines_Dict):
        ...
        List_ratios.append(Lines_Dict['S3_9531A'] / Lines_Dict['S3_9069A'])
    else:
...
with doc.create(Tabular(table_format)) as table:
    ...
    for i in range(len(FilesList)):
        ...
        #Load the data
        Ratios_dict, RatiosColor_dict = load_Galaxy_Ratios(FileFolder, CodeName, '_WHT_LinesLog_v3.txt', Atom_Dict)
...
#             Treatment for the line ratios
            ...
            #Treatment for the inference parameters
                ...
                magnitude   = pv.GetParameter_ObjLog(CodeName, FileFolder, thekey, 'float')

                if magnitude != None:
                    error_type  = thekey[0:thekey.find('_inf')] + '_SD'
                    error       = pv.GetParameter_ObjLog(CodeName, FileFolder, error_type, 'float')
                    value       = ufloat(magnitude,error)
                    color       = colorEvaluator(thekey, value, CodeName, FileFolder)                   
                    formated_abundance = format_for_table(value, 3)
                    Object_row[j] = r'\textcolor{' + color + '}{' + formated_abundance + '}'
                else:
                    formated_abundance = None   
                    Object_row[j] = formated_abundance
...
# 
# print 'all ratios'
# List_ratios_np = array(List_ratios)
# print 'length', len(List_ratios_np)
# print 'mean', mean(List_ratios_np)
# print 'std', std(List_ratios_np)
